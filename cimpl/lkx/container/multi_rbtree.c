...
{
  lkx_RB_M_Node *last_query = tree->last_query;
...
bool lkx_rb_m_insert_k64(lkx_RB_M_Tree *tree, int64 key, lkx_RB_M_Node *node)
{
  ...
  {
    lkx_RB_M_Node *last_next = tree->root;

    while(true)
    {
      if(last_next->data == node->data)
      {
        return false;
      }

      if(last_next->next == NULL)
      {
        last_next->next = node;
        node->prev = last_next;
        return true;
...
check_uncle:
  ...
  lkx_RB_M_Node *grandpa_parent = grandpa->parent;
...
bool lkx_rb_m_erase_k64(lkx_RB_M_Tree *tree, int64 key, lkx_RB_M_Node **node)
{
  lkx_RB_M_Node *cur = tree->root;
  bool left;
  lkx_RB_M_Node *parent, *brother;
  lkx_RB_M_Node null_node = {.color = RB_COLOR_BLACK};
  ...
  while(true)
  {
    ...
    if(key = cur->k64)
    {
      ...
      lkx_RB_M_Node *left_child = cur->left;
      lkx_RB_M_Node *right_child = cur->right;
      *node = cur;
      ...
      {
        lkx_RB_M_Node *r_left = right_child;
  ...
  if(left)
  {
    ...
    if(brother->color == RB_COLOR_BLACK)
    {
      if(brother->right != NULL && brother->right->color == RB_COLOR_RED)
      {
        ...
        {
          lkx_RB_M_Node *grandpa = parent->parent;
